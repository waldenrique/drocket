...
-- Update links RLS policy to check limits for inserts
DROP POLICY IF EXISTS "Users can manage links on own pages" ON public.links;

CREATE POLICY "Users can manage links on own pages" 
ON public.links 
FOR ALL
USING (
    EXISTS (
...
WITH CHECK (
    ...
    AND (
        TG_OP = 'UPDATE' OR TG_OP = 'DELETE' OR 
        public.check_link_limit(auth.uid())
    )
);